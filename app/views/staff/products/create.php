<div class="course-form-container">
    <div style="margin-bottom: 30px;">
        <h2 style="font-size: 24px; font-weight: 600; color: #333; margin: 0;">เพิ่มสินค้าใหม่</h2>
        <p style="font-size: 14px; color: #888; margin-top: 5px;">กรอกข้อมูลรายละเอียดสินค้าและจัดการสต็อก</p>
    </div>

    <form method="POST" action="index.php?action=staff_product_store" enctype="multipart/form-data">

        <div class="form-group">
            <label class="form-label">ชื่อสินค้า</label>
            <input type="text" name="name" class="form-input" placeholder="ระบุชื่อสินค้า (เช่น สครับขมิ้น)" required>
        </div>

        <div class="form-group">
            <label class="form-label">รายละเอียดสินค้า</label>
            <textarea name="description" class="form-textarea" rows="4" placeholder="ระบุคุณสมบัติ, ส่วนผสม หรือวิธีใช้..." required></textarea>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            ...
            <div class="form-group">
                ...
                <input type="text" name="sku" class="form-input" placeholder="เช่น PROD-001">
            </div>
        </div>
            ...
            <div class="form-group">
                ...
                <div class="input-group-row">
                    <select name="product_type" id="product_type_select" class="form-select">
                        <option value="Body Scrub">สครับขัดผิว (Body Scrub)</option>
                        <option value="Body Mask">มาสก์พอกตัว (Body Mask)</option>
                        <option value="Body Massage Oil">น้ำมันนวดตัว (Body Massage Oil)</option>
                        <?php if (isset($product['product_type']) && !in_array($product['product_type'], ['Body Scrub', 'Body Mask', 'Body Massage Oil', 'consumable', 'equipment'])): ?>
                            <option value="<?= htmlspecialchars($product['product_type']) ?>" selected><?= htmlspecialchars($product['product_type']) ?></option>
                        <?php endif; ?>
                    </select>

                    <button type="button" class="btn-add-type" onclick="addNewOption('product_type_select')" title="เพิ่มประเภทใหม่">+</button>
                </div>
            </div>
        </div>
        ...
        <div class="form-actions">
            <a href="index.php?action=staff_product_list" class="btn-back">ย้อนกลับ</a>
